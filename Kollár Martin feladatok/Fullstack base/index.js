...
const Vizsga = mongoose.model('Vizsga', {
    nev: String,
...
app.post('/fesztivalhozzaad', (req, res) => {
  ...
  const osszeg = req.body.osszeg;
    ...
    const fesztivalInstance = new Vizsga({ 
      nev,
      szuletesi_datum,
      telefonszam,
      email,
...
app.get('/fesztivalkiir', (req, res) => {
    Vizsga.find()
      ...
      .catch(error => {
        console.log('Hiba a rekordok lekérdezésekor:', error);
        res.status(500).send('Hiba történt a rekordok lekérdezésekor');
      });
});
...
app.post('/fesztivaltorol/:id', (req, res) => {
  ...
  Vizsga.findByIdAndDelete(fesztivalId)
      .then(fesztival => {
        res.send({ fesztival });
      })
      .catch(error => {
        console.log('Hiba a rekord törlésekor:', error);
        res.status(500).send('Hiba történt a rekord törlésekor');
      });
});
